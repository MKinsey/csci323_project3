                ...
                link: function(scope, ele, attrs) {
                    d3Service.d3().then(function(d3) {
                        ...
                        svg_c.call(window.zoom.scaleExtent([minZoom,maxZoom]).on("zoom", function() {
                                    if(scope.userType < 2 && scope.enableViewportSync) {
                                        scope.enableViewportSync = false;
                                    }
                        ...
                        var doResize = function() {
                            var mobileValue = 0;
                            if($('.device-sm').is(':visible')) mobileValue = 215;
                            if($('.device-xs').is(':visible')) mobileValue = 200;
                            svg_c.style('height', window.innerHeight - heightTrim - mobileValue);
                        ...
                        scope.updatePositions = function(circles) {
                            ...
                            circles.attr("r", function(d) {
                                return (Math.sqrt(d.m)*sizeFactor)+5
                            });
                        ...
                        scope.drag = d3.behavior.drag().origin(function(d) { return d }).on("dragstart", function(d) {
                                d3.event.sourceEvent.stopPropagation();
                                    ...
                                    console.log("d.userID: " + d.userID + " | scope.name: " + scope.name + " | scope.userType: " + scope.userType);
                                    if( !scope.running && ( scope.userType == 2 || ( (scope.name == d.userID || d.userID == null) && scope.userType == 1 )   )  ) { // TODO: add permission check
                                        scope.$apply(function() {
                        ...
                        scope.render = function(data) {
                            ...
                            renderTimeout = $timeout(function() {
                                ...
                                circleEnter.attr("opacity", "0.75").call(scope.drag);
                                circleEnter.on("mousedown", function(d) { d3.event.stopPropagation(); console.log("clicked", d); d3.selectAll("circle").style("opacity", "0.2"); d3.select(this).style("opacity", "1"); prepareToDeselect = false; scope.$apply(function() { scope.currentBody = d; }) });
                                circleEnter.on("touchstart", function(d) { d3.event.stopPropagation(); console.log("clicked", d); d3.selectAll("circle").style("opacity", "0.2"); d3.select(this).style("opacity", "1"); prepareToDeselect = false; scope.$apply(function() { scope.currentBody = d; }) });
                                svg_c.on("mousedown",function() {prepareToDeselect = true});
                                svg_c.on("touchstart",function() {prepareToDeselect = true});
                                svg_c.on("mouseup",function() {if(prepareToDeselect) {console.log("clicked");svg.selectAll("circle").style("opacity", "0.75"); scope.$apply(function() { scope.currentBody = null; });}});
                                svg_c.on("touchend", function() {if(prepareToDeselect) {console.log("clicked");svg.selectAll("circle").style("opacity", "0.75"); scope.$apply(function() { scope.currentBody = null; });}});
